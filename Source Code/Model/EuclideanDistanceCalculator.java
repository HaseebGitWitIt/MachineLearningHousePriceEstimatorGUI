 ...
 * - A distance calculator that uses a euclidean distance method
 * 
 */
public class EuclideanDistanceCalculator extends DistanceCalculator{
	 ...
	 */
	public EuclideanDistanceCalculator(knnComparible obj1, knnComparible obj2, String keyToExclude) {
	...
	public void calcDistance(knnComparible obj1, knnComparible obj2, String keyToExclude) {
			...
			while (n < obj1Vals.size()) {
				if (!obj1Names.get(n).equals(keyToExclude)) {
					if ((obj1Vals.get(n) != null) && (obj2Vals.get(n) != null)) {
						distVal = distVal + Math.pow(obj1Vals.get(n) - obj2Vals.get(n), 2);
					} else {					
		...
		distVal = Math.sqrt(distVal); // Square root the distance
		super.distance = distVal; // Set the new distance
	}

	/**
	* Calculates the distance between two ArrayLists using this classes distance metric
		...
		if (set1.size() >= set2.size()) {
			while (i < set2.size()) {
				if ((set1.get(i) != null) && (set2.get(i) != null)) {
					toReturn = toReturn + Math.pow(set1.get(i) - set2.get(i), 2);
				} else {
