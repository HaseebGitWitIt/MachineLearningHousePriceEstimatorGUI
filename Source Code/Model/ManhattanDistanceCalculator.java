 ...
 * - A distance calculator that uses a Manhattan distance method
 * - Distance = Sum of the absolute value of (xi - yi)
 * 
 */
public class ManhattanDistanceCalculator extends DistanceCalculator {
	 ...
	 */
	public ManhattanDistanceCalculator(knnComparible obj1, knnComparible obj2, String keyToExclude) {
			...
			while (n < obj1Vals.size()) {
				if (!obj1Names.get(n).equals(keyToExclude)) {
					if ((obj1Vals.get(n) != null) && (obj2Vals.get(n) != null)) {
						distVal = distVal + Math.abs(obj1Vals.get(n) - obj2Vals.get(n));
					} else {
	...
	/**
	* Calculates the distance between two ArrayLists using the manhatten distance metric
		...
		if (set1.size() >= set2.size()) {
			while (i < set2.size()) {
				if ((set1.get(i) != null) && (set2.get(i) != null)) {
					toReturn = toReturn + Math.abs(set1.get(i) - set2.get(i));
				} else {
